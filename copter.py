...
class YOLO(object):
    _defaults = {
        ...
        "model_path": 'model_data/007/trained_weights_final_007.h5',        
        "anchors_path": 'model_data/007/yolo_anchors_007.txt',
        "classes_path": 'model_data/voc_classes_006.txt',
        "score" : 0.4,
        "iou" : 0.45,
...
def goto(target, gotoFunction = vehicle.simple_goto):
    ...
    gotoFunction(location=target ,groundspeed=1)
    ...
    while vehicle.mode.name=="GUIDED": #Stop action if we are no longer in guided mode.
        ...
        #print('[lat,lon]',cur_pos.lat,cur_pos.lon)
        message = json.dumps(data)
        producer.produce(message.encode('ascii'))               
        
        remainingDistance = haversine(cur_pos,target.lon,target.lat)
        print("[ Distance to target ]:{:.2f} (m), [ Speed ]:{:.2f} (m/s)".format(remainingDistance,vehicle.groundspeed))    
        if remainingDistance>50 :
            vehicle.groundspeed = 15
        if remainingDistance<50:
            vehicle.groundspeed = 12
        ...
        if remainingDistance<=targetDistance*torlerance: #Just below target, in case of undershoot.
...
def COPTER_JOB():   
    ...
    arm_and_takeoff(aTargetAltitude=10)
    try:   
        for i in range(len(waypoints)): 
            ...
            print("------------------------------------------------------")     
            target = LocationGlobalRelative(lat=waypoints[i][1],lon=waypoints[i][0],alt=20)
            time.sleep(1)   
            goto(target)  
...
if __name__ == '__main__':   
    ...
    waypoints = read_json('./data/Fishing_port.json')
